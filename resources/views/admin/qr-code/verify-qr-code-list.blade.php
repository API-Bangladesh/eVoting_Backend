...
@section('title', 'All Verified QR Codes')

@section('content')
    <main id="dashboard">
        <div class="container-fluid">
            <h3 class="fs-3 fw-normal mb-4">Verify Ballot Scan</h3>

            @include('common.alert-message')

            <div class="eVote-table p-4 is-drop-shadow bg-white rounded">
                <div class="row align-items-center justify-content-sm-between mb-4">
                    <form action="{{ route('verify-qr-code') }}" method="POST"
                        ...
                        <input type="password" name="scan_input" id="scan_input"
                               class="form-control rounded-pill flex-grow-1"
                               placeholder="Cursor should be pointed here.." autofocus/>
                    </form>
                ...
                <div class="voter-list table-responsive">
                    <table class="table table-sm align-middle">
                        ...
                        <tr>
                            ...
                            <th>Voted Ballot Scan Date-Time</th>
                        </tr>
                        </thead>
                        <tbody>
                        @foreach($qrCodes as $key => $qrCode)
                            <tr>
                                ...
                                <td>{{ optional($qrCode->verifiedBy)->name }}</td>
                                <td>
                                    @if(empty($qrCode->scan_voted_ballot))
                                ...
                                <td>{{ $qrCode->scan_voted_ballot }}</td>
                            </tr>
                        @endforeach
                        </tbody>
                    </table>
